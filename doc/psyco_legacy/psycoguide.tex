...
\item In Python 2.6 parts of the \module{warnings} module were rewritten in C.  As a result, the reported warnings may be misplaced (they cannot show a Psyco-compiled frame).

\item In Python 2.6, creating namedtuple classes (not instances) in a Psyco-optimized function gives the class the wrong \member{__module__} attribute, for the same reason as above: it is created by C code, so it ignores the Psyco frame hooks.
...
\begin{tableiii}{cl|l}{code}{Bytecode}{Instruction name}{Appears in}
  \lineiii{ 81}{\code{WITH_CLEANUP }}{(6) with statements}
  ...
  \lineiii{104}{\code{BUILD_SET    }}{(7) sets syntax}
  ...
  \lineiii{143}{\code{SETUP_WITH   }}{(6) with statements}
  \lineiii{146}{\code{SET_ADD      }}{(7) sets syntax}
\end{tableiii}
...
\item[(6)]
  The \code{with} statement is not supported.
\item[(7)]

...
\date{updated Aug 27, 2004}

\release{1.2}                   % release version
...
\warning{The Linux binaries are compiled for the relatively recent \file{glibc-2.3}, which means that they will not work on older systems!  Note that it is not more difficult to install from the source than from binaries -- basically, just do \code{python setup.py install} in both cases.} Also note that at least some versions of Psyco have been successfully used on other PC Unixes like FreeBSD.
...
\begin{tableiii}{l|l|l}{filenq}{File name}{Python versions}{Well-tested with}
  \lineiii{ psyco-x.y-OS-2.1     }{2.1 or any 2.1.x}       {2.1.3}
  \lineiii{ psyco-x.y-OS-2.2.0-1 }{2.2 older than 2.2.2}   {2.2.1}
  \lineiii{ psyco-x.y-OS-2.2.2   }{2.2.2 and up}           {2.2.2 and 2.2.3}
  \lineiii{ psyco-x.y-OS-2.3     }{2.3 and up}             {2.3 and 2.3.3}
\end{tableiii}

\file{x.y} is the Psyco version number, and \file{OS} is the operating system this version has been compiled for.

The binaries contain a subdirectory named \file{psyco} which is a Python package (which you can check by looking for the \file{__init__.py} file it contains).  Just drop this directory into your \file{site-package} directory.  Note that you should not rename the main directory called \file{psyco-1.x}!  If you do it right, you will have a file with precisely the following path: \file{.../python2.x/lib/site-packages/psyco/core.py}.

% XXX setup.py not included in binary distributions version 1.2
...
Alternatively you can get it from its most current CVS version; see \url{http://sourceforge.net/cvs/?group_id=41036}. People unfamiliar with CVS or whose CVS access is firewalled can use \program{cvsgrabber} (\url{http://cvsgrab.sourceforge.net}) to download the current source via HTTP.
...
will compile the C source and put the result directly into the \file{psyco/} subdirectory (no administrator priviledge is required). After this step, the \file{psyco/} directory is a complete package (exactly as found in the binary distributions) that you can use directly or copy around.
...
\item[(2)]
  Functions performing dynamic evaluations cannot see the current locals.  You should explicitely supply a globals and/or locals dictionary to these functions.  They issue a \exception{psyco.warning} if you don't.  Use \function{raw_input} and the three-arguments form of \function{eval} instead of \function{input}.
...
\begin{tableiii}{cl|l}{code}{Bytecode}{Instruction name}{Appears in}
  ...
  \lineiii{ 84}{\code{IMPORT_STAR  }}{(2) \code{from xx import *}}
  \lineiii{ 85}{\code{EXEC_STMT    }}{(2) \code{exec xx}}
  \lineiii{ 86}{\code{YIELD_VALUE  }}{(3) generators}
  \lineiii{ 90}{\code{STORE_NAME   }}{(1) outside functions}
  \lineiii{ 91}{\code{DELETE_NAME  }}{(1) outside functions}
  \lineiii{101}{\code{LOAD_NAME    }}{(1) outside functions}
...
\begin{description}
\item[(1)]
  Psyco can only accelerate functions.  It can not accelerate class definitions (i.e.\ the execution of the body of the class statement -- i.e.\ the creation of the class object itself -- this does not prevent it from accelerating methods in the class).  It cannot accelerate the module top-level code, so be sure to put your loops inside a function even in a simple script.  These instructions can also sometimes appear in a function, e.g.\ when the function uses \code{from xx import *} or \code{exec xx}.
\item[(2)]
  Functions using these constructs cannot be accelerated.
\item[(3)]
  Generators (i.e.\ any function using the \code{yield} keyword) cannot be accelerated currently.  If there is enough interest I can consider implementing them.
\item[(4)]

...
\item the code defining a class\footnote{The methods themselves are fine; this only concerns the code that runs in order to define the class (generally only a bunch of \code{def} statements)} (top-level module code can be executed by recent versions of Psyco).
...
\begin{tableiii}{cl|l}{code}{Bytecode}{Instruction name}{Appears in}
  ...
  \lineiii{ 90}{\code{STORE_NAME   }}{(5) class definitions}
  \lineiii{ 91}{\code{DELETE_NAME  }}{(5) class definitions}
  \lineiii{101}{\code{LOAD_NAME    }}{(5) class definitions}
...
\item[(5)]
  These constructions are allowed when they appear at the module top-level, i.e.\ not in a function or in a class definition.
\end{description}

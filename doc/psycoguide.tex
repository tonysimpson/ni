...
\release{1.0.0b1}                   % release version
...
\item \strong{A Pentium or any other Intel 386 compatible processor.}  Sorry, no other processor is supported yet.
...
\begin{tableii}{l|l}{filenq}{File name}{Python version}
  \lineii{ psyco-x.y.z-OS-2.1     }{2.1 or any 2.1.x}
  \lineii{ psyco-x.y.z-OS-2.2.0-1 }{2.2 older than 2.2.2}
  \lineii{ psyco-x.y.z-OS-2.2.2   }{2.2.2 and up}
\end{tableii}

\file{x.y.z} is the Psyco version number, and \file{OS} is the operating system this version has been compiled for.
...
The binaries contain a subdirectory named \file{psyco} which is a Python package (which you can check by looking for the \file{__init__.py} file it contains).  Just drop this directory into your \file{site-package} directory.  In other words, you need a directory \file{.../python2.x/lib/site-packages/psyco/} which contains the files \file{__init__.py}, \file{core.py}, and so on.

Alternatively, run the top-level installation script \file{setup.py} as follows:
...
People unfamiliar with CVS or whose CVS access is firewalled can use \program{cvsgrabber} (\url{http://cvsgrab.sourceforge.net}) to download the current source via HTTP.
...
to compile it without installing it into the \file{site-packages} directory.
...
On a Dual Pentium Linux running Python 2.2.2, using the Zope 2.6 distribution, I get the following times:

\begin{verbatim}
\fixme
\end{verbatim}

If I add the following line at the top of \file{markbench.py} the times drop massively --- the longest-running tests run as much as three times faster!
...
import psyco; psyco.log(); psyco.bind(TALInterpreter)
\end{verbatim}
...
Here you can see that only the TAL test is accelerated.  If you have a Zope server using TAL, you could try to add the above two lines to it.  You may get very interesting speed-ups for a very reasonable memory overweight (please tell me about it --- there is no real Zope server I can play with myself)!
...
\begin{funcdesc}{profile}{watermark=0.09, halflife=0.5, pollfreq=20, parentframe=0.25, memory=None, time=None, memorymax=None, timemax=None}
  ...
  As far as I can tell, the best solution for large applications.  With Python 2.2.2, if a function takes a long time to run, it will be detected and compiled \emph{while it is still running,} in the middle of its execution.  Psyco takes it away from the standard interpreter.
...
\begin{funcdesc}{cannotcompile}{object}
  Prevents the given function to be ever compiled by Psyco.  \var{object} can also be a method object or directly a code object.  Raises \exception{psyco.error} if the object is already being compiled.  Note that this does not prevent functions called by \var{object} to be compiled, if the profilers want them to.
\end{funcdesc}
...
With Python 2.2.2 you may try to add a line \samp{psyco.runonly()} after the last profiler, if all your profilers may stop (i.e.\ all of them have limits).  It might help, or it might not.  Feedback appreciated.
...
\item \strong{No locals dictionary} is available.  The \function{locals} function always return an empty dictionary (and issues a warning).  This also affects \function{eval}, \function{execfile}, \function{vars}, \function{dir} and \function{input} unless you supply an explicit dictionary for the locals.  See section \ref{patchedfunctions}.
  
\item \strong{Frame and traceback objects} are only partially visible to Python code.  If you use functions like \function{sys._getframe} to obtain a frame object that corresponds to a Psyco-accelerated frame, some of its fields will be empty, most notably \member{f_locals} and \member{f_back}.  Actually, only the \member{f_code} and \member{f_globals} fields are well-tested.
...
\begin{tableii}{c|l}{function}{Built-in function}{Notes}
  ...
  \lineii{ eval          }{(2) when called with a single argument}
  \lineii{ execfile      }{(2) when called with a single argument}
  \lineii{ input         }{(2)}
...
\begin{description}
\item[(1)]
  A function run by Psyco has no \function{locals} dictionary.  Trying to access it returns an empty dictionary and issues a \exception{psyco.warning}.
\item[(2)]
  Functions performing dynamic evaluations cannot see the current locals.  You should explicitely supply a globals and/or locals dictionary to these functions.  They issue a \exception{psyco.warning} if you don't.  Use \function{raw_input} and \function{eval} instead of \function{input}.
\item[(3)]
...
\begin{tableiii}{cl|l}{code}{Bytecode}{Instruction name}{Appears in}
  \lineiii{ 82}{\code{LOAD_LOCALS  }}{class definitions}
  ...
  \lineiii{136}{\code{LOAD_DEREF   }}{nested scopes}
...
\begin{tableiii}{c|ll}{textrm}{Type}{Operations}{Notes}
  ...
  \lineiii{Sequence iterators}{\code{iter()} and \code{next()}}{(7)}

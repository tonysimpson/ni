...
#define RunTime_RegMask 0xFFFFFFFFF0000000
...
#if REG_TOTAL > 0
PSY_INLINE reg_t getreg(RunTimeSource s) {
  CHKTIME(s, RunTime);
  return (reg_t)(s >> 28);
}
#endif
PSY_INLINE bool is_reg_none(RunTimeSource s) {
  CHKTIME(s, RunTime);
  return s < 0;
}
PSY_INLINE long getstack(RunTimeSource s) {
  CHKTIME(s, RunTime);
...
PSY_INLINE bool is_rtnonneg(RunTimeSource s) {
  CHKTIME(s, RunTime);
  return s & RunTime_NonNeg;
}

/* mutation */
PSY_INLINE RunTimeSource remove_rtref(RunTimeSource s) {
  CHKTIME(s, RunTime);
  return s | RunTime_NoRef;
}
PSY_INLINE RunTimeSource add_rtref(RunTimeSource s) {
  CHKTIME(s, RunTime);
  return s & ~RunTime_NoRef;
}
#if REG_TOTAL > 0
...
PSY_INLINE void sk_incref(source_known_t *sk) {
  sk->refcount1_flags += SkFlagEnd;
}
PSY_INLINE void sk_decref(source_known_t *sk) {
  if ((sk->refcount1_flags -= SkFlagEnd) < 0)
    sk_release(sk);
}
...
PSY_INLINE void sk_delete(source_known_t *sk) { psyco_llfree_sk(sk); }
...
PSY_INLINE CompileTimeSource set_ct_value(CompileTimeSource s, long v) {
  ...
  if (sk->refcount1_flags < SkFlagEnd) {
    ...
    sk_decref(sk);
    return CompileTime_NewSk(sk_new(v, sk->refcount1_flags & SkFlagMask));
  }
}
...
#define INIT_SVIRTUAL(sv, fn, directfn, objmask, w_normal, w_funcall)          \
  do {                                                                         \
    (sv).compute_fn = fn;                                                      \
...
PSY_INLINE bool vinfo_known_equal(vinfo_t *v, vinfo_t *w) {
  return (v->source == w->source && (v == w || !is_virtualtime(v->source)));
}
...
PSY_INLINE bool is_nonneg(Source s) {
  switch (gettime(s)) {
  case RunTime:
    return is_rtnonneg(s);
  case CompileTime:
    return CompileTime_Get(s)->value >= 0;
  default:
    return false;
  }
}
...
PSY_INLINE void vinfo_setitem(PsycoObject *po, vinfo_t *vi, int index,
  ...
  if (newitem != NULL) {
    extra_assert(
        !(is_compiletime(vi->source) && !is_compiletime(newitem->source)));
  }
  vinfo_array_grow(vi, index + 1);
...
PSY_INLINE void deallocate_array(vinfo_array_t *array, PsycoObject *po) {
  int i = array->count;
  while (i--)
    vinfo_xdecref(array->items[i], po);
}
       ...
       : sizeof(ctype) == 2                                                    \
             ...
             : sizeof(ctype) == 4                                              \
                   ...
                   : sizeof(ctype) == 8                                        \
                         ...
                         : sizeof(ctype) == 16                                 \
                               ? 4                                             \
                               : (extra_assert(!"field size is not a small "   \
                                                "power of two"),               \
                                  0))
...
#define STRUCT_FIELD_BUILD(cstruct, ctype, cfield, prevf, flags)               \
  ((defield_t)((offsetof(cstruct, cfield) << FIELD_OFS_SHIFT) |                \
               ...
               field_next_index(prevf, true) | flags))
#define ARRAY_FIELD_BUILD(ctype, baseofs, flags)                               \
  ((defield_t)(((long)(baseofs) << FIELD_OFS_SHIFT) |                          \
               (SIZE2_FROM_CTYPE(ctype) << FIELD_SIZE2_SHIFT) |                \
               FIELD_RESERVED_INDEX | FIELD_ARRAY | flags))
...
#define DEF_ARRAY(ctype, baseofs) ARRAY_FIELD_BUILD(ctype, baseofs, 0)
...
#define FPYREF(df)                                                             \
  (extra_assert(1 << FIELD_SIZE2(df) == sizeof(PyObject *)),                   \
...
#define CHECK_FIELD_INDEX(n)                                                   \
  extra_assert((unsigned)FIELD_INDEX(n) < FIELD_RESERVED_INDEX)

/* functions to read or write a field from or to the structure
   pointed to by 'vi': */
PSY_INLINE vinfo_t *psyco_get_field(PsycoObject *po, vinfo_t *vi,
                                    defield_t df) {
  return psyco_internal_getfld(po, FIELD_INDEX(df), df, vi, FIELD_C_OFFSET(df));
}
PSY_INLINE vinfo_t *psyco_get_nth_field(PsycoObject *po, vinfo_t *vi,
                                        defield_t df, int index) {
  long ofs = index << FIELD_SIZE2(df);
  return psyco_internal_getfld(po, FIELD_INDEX(df) + index, df, vi,
                               FIELD_C_OFFSET(df) + ofs);
}
PSY_INLINE vinfo_t *psyco_get_field_offset(PsycoObject *po, vinfo_t *vi,
  ...
  return psyco_internal_getfld(po, FIELD_RESERVED_INDEX, df, vi,
                               FIELD_C_OFFSET(df) + offset);
}
...
PSY_INLINE bool psyco_put_field(PsycoObject *po, vinfo_t *vi, defield_t df,
                                vinfo_t *value) {
  return psyco_internal_putfld(po, FIELD_INDEX(df), df, vi, FIELD_C_OFFSET(df),
                               value);
}
PSY_INLINE bool psyco_put_nth_field(PsycoObject *po, vinfo_t *vi, defield_t df,
                                    int index, vinfo_t *value) {
  long ofs = index << FIELD_SIZE2(df);
  return psyco_internal_putfld(po, FIELD_INDEX(df) + index, df, vi,
                               FIELD_C_OFFSET(df) + ofs, value);
}
...
#define FIELD_PART2(df)                                                        \
  ((defield_t)((long)(df) + 1 +                    /* next index */            \
...
PSY_INLINE vinfo_t *psyco_get_const(PsycoObject *po, vinfo_t *vi,
                                    defield_t df) {
  return psyco_internal_getfld(po, FIELD_INDEX(df),
                               (defield_t)((long)df | FIELD_INTL_NOREF), vi,
                               FIELD_C_OFFSET(df));
}
PSY_INLINE vinfo_t *psyco_get_nth_const(PsycoObject *po, vinfo_t *vi,
                                        defield_t df, int index) {
  long ofs = index << FIELD_SIZE2(df);
...
PSY_INLINE void psyco_forget_nth_field(PsycoObject *po, vinfo_t *vi,
                                       defield_t df, int index) {
  CHECK_FIELD_INDEX(df);
...
EXTERNFN void psyco_coding_pause(PsycoObject *po, condition_code_t jmpcondition,
                                 resume_fn_t resume_fn, void *extra,
                                 size_t extrasize);
...
#else
#define psyco_assert_coherent1(po, full)                                       \
  do {                                                                         \
  } while (0) /* nothing */
#endif
...
EXTERNFN mergepoint_t *PsycoObject_Ready(PsycoObject *po);
EXTERNFN PsycoObject *PsycoObject_FromCode(PyCodeObject *co, PyObject *globals,

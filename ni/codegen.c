...
void psyco_emit_header(PsycoObject *po, int nframelocal) {
  ...
  while (j--)
    array->items[j] = vinfo_new(
        RunTime_NewStack(po->stack_depth - sizeof(long) * j, false, false));
}
...
EXTERNFN vinfo_t *compile_time_call(PsycoObject *po, void *c_function,
                                    int flags, int arg_count, char argtags[], long args[]);
EXTERNFN vinfo_t *run_time_call(PsycoObject *po, void *c_function, int flags,
                                int arg_count, char argtags[], long args[],
                                int total_output_array_size);

#define MAX_ARGUMENTS_COUNT 7

DEFINEFN
vinfo_t *psyco_generic_call(PsycoObject *po, void *c_function, int flags,
  ...
  for (count = 0; arguments[count]; count++) {
    ...
    case 'A':
      has_refs = true;
      totalstackspace += sizeof(long) * ((vinfo_array_t *)arg)->count;
      break;
    default:
  ...
  if (flags & CfPure) {
    ...
    vresult = compile_time_call(po, c_function, flags, count, argtags, args);
    if (has_refs) {
      if (vresult == NULL) {
        for (i = 0; i < count; i++) {
          if (argtags[i] == 'a' || argtags[i] == 'A') {
        ...
        for (i = 0; i < count; i++) {
          if (argtags[i] == 'a' || argtags[i] == 'A') {
            ...
            for (j = 0; j < array->count; j++) {
              array->items[j] = vinfo_new(
                  CompileTime_NewSk(sk_new(((long *)args[i])[j], sk_flag)));
            }
            free((void *)args[i]);
          }
        }
...
/* forward */
static condition_code_t int_cmp_i(PsycoObject *po, vinfo_t *rt1, long immed2,
                                  int py_op);
...
vinfo_t *integer_or(PsycoObject *po, vinfo_t *v1, vinfo_t *v2) {
  ...
  return BINARY_INSTR_OR(false, is_nonneg(v1->source) && is_nonneg(v2->source));
}
...
static vinfo_t *int_mul_i(PsycoObject *po, vinfo_t *v1, long value2, bool ovf) {
  switch (value2) {
  case 0:
...
DEFINEFN
condition_code_t integer_cmp(PsycoObject *po, vinfo_t *v1, vinfo_t *v2,
                             int py_op) {
  int base_py_op = py_op & COMPARE_OP_MASK;
...
DEFINEFN
condition_code_t integer_cmp_i(PsycoObject *po, vinfo_t *v1, long value2,
                               int py_op) {
  if (!compute_vinfo(v1, po))
    return CC_ERROR;

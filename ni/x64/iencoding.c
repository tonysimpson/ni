...
DEFINEFN void *psyco_call_code_builder(PsycoObject *po, void *fn, int restore,
  ...
  PUSH_R(REG_FUNCTIONS_RETURN); /* TODO: Fix
    Need to preserve RAX because we can be here from run_time_result_check and
    we don't decided to put RAX in a compiler tracked location until
    run_time_result. */
...
static long var_call(void *c_function, int flags, int arg_count, char argtags[],
                     long args[]) {
...
DEFINEFN vinfo_t *compile_time_call(PsycoObject *po, void *c_function,
                                    int flags, int arg_count, char argtags[],
                                    long args[]) {
  vinfo_t *vresult;
  long result;
...
PSY_INLINE vinfo_t *new_rtvinfo(PsycoObject *po, reg_t reg, bool ref,
                                bool nonneg) {
...
static vinfo_t *run_time_result(PsycoObject *po, int flags) {
  ...
  if ((flags & CfReturnTypeMask) == CfReturnTypeInt) {
    if (flags & CfReturnUnsigned) {
      MOVZX_QR_DR(
          rg, REG_X64_RAX); /* mov unsigned (zero extended) from EAX to rg */
    } else {
      MOVSX_QR_DR(rg,
...
DEFINEFN vinfo_t *run_time_call(PsycoObject *po, void *c_function, int flags,
  ...
  for (i = arg_count, double_arg_i = double_arg_count - 1; i--;) {
    ...
    case 'A': {
      ...
      while (j > 0) {
        ...
        array->items[--j] =
            vinfo_new(RunTime_NewStack(stackbase, with_reference, false));
      }
...
vinfo_t *psyco_call_psyco(PsycoObject *po, CodeBufferObject *codebuf,
  ...
  do {
    stack_correction = (16 - (((argcount * sizeof(long)) +
                               STACK_DEPTH_SINCE_CALL() + INITIAL_STACK_DEPTH) %
                              16)) %
                       16;
...
DEFINEFN
vinfo_t *bininstrshift(PsycoObject *po, int group, bool nonneg, vinfo_t *v1,
                       vinfo_t *v2) {
  reg_t rg;
  BEGIN_CODE
...
DEFINEFN
vinfo_t *bint_lshift_i(PsycoObject *po, vinfo_t *v1,
...
DEFINEFN
vinfo_t *bininstrmul(PsycoObject *po, bool ovf, bool nonneg, vinfo_t *v1,
                     vinfo_t *v2) {
  reg_t rg;
  BEGIN_CODE
  NEED_CC();
...
DEFINEFN
vinfo_t *unaryinstrgrp(PsycoObject *po, int group, bool ovf, bool nonneg,
                       vinfo_t *v1) {
  reg_t rg;
  BEGIN_CODE
  NEED_CC();
...
DEFINEFN
vinfo_t *unaryinstrabs(PsycoObject *po, bool ovf, bool nonneg, vinfo_t *v1) {
  reg_t rg;
  BEGIN_CODE
  NEED_CC();
  ...
  NEED_FREE_REG_COND(
      rg, (rg != REG_X64_RAX && rg != REG_X64_RDX && rg != getreg(v1->source)));
...
DEFINEFN
condition_code_t bint_cmp_i(PsycoObject *po, int base_py_op, vinfo_t *rt1,
                            long immed2) {
  BEGIN_CODE
  NEED_CC();
...
DEFINEFN
condition_code_t bininstrcmp(PsycoObject *po, int base_py_op, vinfo_t *v1,
                             vinfo_t *v2) {
  BEGIN_CODE
  NEED_CC();
...
DEFINEFN
vinfo_t *bininstrcond(PsycoObject *po, condition_code_t cc, long immed_true,
                      long immed_false) {
  reg_t rg;
  BEGIN_CODE
  NEED_FREE_REG(rg);

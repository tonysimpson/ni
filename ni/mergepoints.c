...
PyObject* psyco_get_merge_points(PyCodeObject* co, int module) {
    ...
    mergepoints = PyDict_GetItem(co_to_mp, (PyObject*)co);
    if (mergepoints == NULL) {
        mergepoints = psyco_build_merge_points(co, module);
        PyDict_SetItem(co_to_mp, (PyObject*)co, mergepoints);
    }
    return mergepoints;
}
...
DEFINEFN
mergepoint_t* psyco_exact_merge_point(PyObject* mergepoints, int position)
{
  mergepoint_t* mp;
